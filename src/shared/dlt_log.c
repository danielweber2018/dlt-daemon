 ...
 *
 * \copyright Copyright © 2022 BMW AG. \n
...
DltLoggingMode logging_mode = DLT_LOG_TO_CONSOLE;
...
void dlt_log_init_multiple_logfiles_support(const DltLoggingMode mode, const bool enable_multiple_logfiles,
                                            const int logging_file_size, const int logging_files_max_size)
{
    if (!dlt_is_file_logging_enabled(mode)) return;
...
void dlt_log_init_single_logfile()
{
